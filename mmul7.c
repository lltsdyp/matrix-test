                ...
                for (int i_offset = 0; i_offset < BLOCK_SIZE; ++i_offset)
                    for (int k_offset = 0; k_offset < BLOCK_SIZE; ++k_offset)
                        for (int j_offset = 0; j_offset < BLOCK_SIZE; j_offset += 4)
                        {
                            ...
                            __m256i vector_b = _mm256_load_epi64((void *)&matrixB[k_base * BLOCK_SIZE + k_offset][j_base * BLOCK_SIZE + j_offset]);
                            __m256i outputVector = _mm256_load_epi64((void *)&outputMatrix[i_base * BLOCK_SIZE + i_offset][j_base * BLOCK_SIZE + j_offset]);
                            __m256i product = _mm256_mullo_epi64(vector_a, vector_b);
                            outputVector = _mm256_add_epi64(outputVector, product);
                            _mm256_store_epi64((void *)&outputMatrix[i_base * BLOCK_SIZE + i_offset][j_base * BLOCK_SIZE + j_offset], outputVector);
                        }
